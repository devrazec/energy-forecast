...
def generate_realtime_consumption(base_consumption=13.0, duration_minutes=150, variation_range=3.0):
    ...
    for i in range(total_intervals):
        ...
        minute_key = f"{current_hour:02d}:{current_minute:02d}"
        ...
        if i == 0:
            ...
            wave_offset = np.sin(i / 20) * 1.5  # Smooth wave pattern
        ...
        # Initialize minute key if not exists
        if minute_key not in realtime_data:
            realtime_data[minute_key] = {}
        
        # Add the data point
        realtime_data[minute_key][timestamp] = f"{consumption:.1f}"
    ...
    all_timestamps = {}
    for minute_key in sorted(realtime_data.keys()):
        for timestamp, value in realtime_data[minute_key].items():
            all_timestamps[timestamp] = value
